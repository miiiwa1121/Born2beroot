...
Debianは安定性が高く、オープンソースでコミュニティ主導で開発されているため、信頼性も非常に高いから。また、パッケージ管理が豊富で、セキュリティの更新が定期的に行われるため、長期運用にも適している。
...
APPArmor (アプリケーション アーマー) は、Linuxというオペレーティングシステムで使われるセキュリティツールです。このツールは、コンピュータの中で動いているアプリケーション（ソフトウェア）が悪いことをしないように見張る役割を果たします。
イメージとしては、APPArmorはアプリケーションごとに「ルールブック」を作って、そのルールに従って動くようにします。例えば、「このアプリケーションはインターネットに接続してもいいけど、ファイルを削除することはできない」といった具合です。
この仕組みのおかげで、悪意のあるソフトウェアがシステムに悪さをするのを防ぐことができます。簡単に言えば、APPArmorはコンピュータの警備員のようなものです。
