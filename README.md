...
- `APT`（Advanced Packaging Tool）は、Debian系のLinuxディストリビューションで使用されるパッケージ管理システムです。シンプルで自動化されており、通常はスクリプトで使用されます。Debianの推奨はaptになっている
- `aptitude`は、APTをより詳細に設定できるようにしたインターフェースであり、より豊富なインターフェースを提供します。具体的には、`aptitude`はコマンドラインインターフェースを提供し、検索やパッケージの依存関係を可視化したり、インタラクティブな操作が可能です。しかし、`aptitude`は`APT`を拡張しより詳細なツールを作成しようとしていましたが、
