...
小さいお弁当箱を選んだけど、おかずがたくさんあって入らない！
逆に、大きすぎるお弁当箱にしたけど、中身はスカスカで無駄！

パソコンのストレージも同じで、普通のパーティションではサイズを変更するのが難しいのです。

- `LVM`はレゴブロックみたいに自由に大きさを変えられる箱です！

普通のパーティション（お弁当箱） → サイズが決まっている！
LVM（レゴブロック） → 後から増やしたり、減らしたりできる！

例えば、レゴブロックを組み立てて「机」を作ったとします。

もっと大きな机にしたい！ → ブロックを追加！
小さくしたい！ → ブロックを外す！
LVM を使えば、パソコンのデータを入れる「箱」も 後から自由に変更 できます。

パソコンの容量を簡単に増やしたり、小さくしたりできるのです！
...
- **`hostnamectl`**: ホスト名を確認したり設定したりするときに使うコマンド
