...
### 仕組み
仕組みとしては、`ホスト型`、`ハイパーバイザー型`、`コンテナ型`と複数ありそれぞれ用途によって使い分けができる。今回の課題はホスト型に当たる。

#### ホスト型
```
- ホストOS上にソフトウェアとして仮想化環境を構築する方式。
<特徴>
- 既存のOS（Windows, macOS, Linuxなど）上で動作する
- 一般的なPCでも手軽に仮想マシンを作成できる
- パフォーマンスはやや劣る（ホストOSを経由するため）
<代表的なソフトウェア>
- VirtualBox
- VMware Workstation
```
...
#### ハイパーバイザー型
```
- 専用のハイパーバイザーが直接ハードウェアを管理し、仮想マシンを実行する方式。
<特徴>
- OSを介さずに直接ハードウェアを制御するため、高速かつ安定
- サーバー向けの仮想化技術として広く利用される
- 専用の管理ツールを使用してリソースを効率的に割り当て可能
<代表的なソフトウェア>
- VMware ESXi
- Microsoft Hyper-V
...
#### コンテナ型
```
- OSのカーネルを共有しながら、アプリケーションごとに独立した環境を構築する方式。
<特徴>
- 仮想マシンよりも軽量で、起動・停止が高速
- OSのカーネルを共有するため、リソース消費が少ない
- ただし、ホストOSと異なるカーネルは使用できない（Linux上でWindowsコンテナは動かせない）
<代表的なソフトウェア>
- Docker
```

### 目的
- 一台の物理マシン上で複数のOS環境を立ち上げる
- 自身のOS異なるOSを使用できる
- 仮想環境をコピーして配布できる
- 環境のリセットを容易にできる
- 効率的にリソースを利用できる
...
### 利点１
普通、パソコンのデータは決まった大きさの箱（パーティション）に入ります。ですが、一度大きさを決めると後から変えられないのが問題です。
そのため入れるデータ量によっては、「パーティションが大き過ぎて無駄が多い」、また「パーティションが小さくて入り切らない」などと言った問題が起きます。
普通のパーティションではサイズを変更するのが難しいのです。

- `LVM`はレゴブロックみたいに自由に大きさを変えられる箱です！

普通のパーティション → サイズが決まっている！
LVM → 後から増やしたり、減らしたりできる！

### 利点２
普通マシンは自身のストレージの中で機能します。ではストレージが不足した場合には、より高性能高価格のストレージを買い、取り替える必要があるのでしょうか？

- `LVM`は複数の物理ストレージ団結させることができる！

実際にはストレージを取り替える必要はなく、追加で拡張させることができます。例えば、500GBのSSDと1TBのHDDを1.5TBのストレージとして扱えます。

LVM を使えば、ストレージを柔軟に操れるのです。

### パーティションとは？
物理的なストレージデバイス（HDDやSSD）や仮想ディスク（VHDやVMDKなど）を論理的に区切った領域のこと

## 9. aptitude と apt (Advanced Packaging Tool) の違いは何ですか?
- まず、`apt`も`aptitude`も`apt-get`の拡張版です。
- それぞれ`apt`はよりシンプルに。`aptitude`はより多機能で便利にという方向性で開発されています。
- `apt`（Advanced Packaging Tool）は、Debian系のLinuxディストリビューションで使用されるパッケージ管理システムです。シンプルで自動化されており、通常はスクリプトで使用されます。Debianの推奨はaptになっています。
- `aptitude`は、インストールしたパッケージが推奨するパッケージなどを自動的に検索して取ってきてくれる。またaptitudeにはGUIモードなどが存在して、ユーザーがよりわかりやすい機能が存在する。不要な自動パッケージの削除など機能が豊富だが、未完成のため非推奨です。
...
Linuxのセキュリティモジュール(セキュリティツール) 
- アプリケーションごとにアクセス制御ポリシーの設定
- システム内で許可された操作を制限
- SELinuxより構成が比較的簡単
- アプリケーションの不正を見張る役割

具体的にはアプリケーションごとにルールブックを作成し、そのルールに従うようにする
<例>appAはインターネットにアクセスしてもいいが、appB はダメやインターネットにはアクセスしてもいいけどファイルの削除はできないなど...

具体的にルールブックは、プロファイルを指す
プロファイルとはルールを指定できる設定ファイル

### プロファイルを作成
`sudo aa-genprof /usr/bin/example-app`
- 実際に動かす、アプリケーションの挙動を記録
...
このようにAPPArmorはプロファイルにルールを指定することで、Linuxのアプリケーションのセキュリティを高めている

## 11. UFW (シンプルなファイアウォール)とは?
`UFW`（Uncomplicated Firewall）は、Linuxで使用される簡単なファイアウォールツールで、`iptables`を簡単に扱えるようにしたインターフェースとして動作します。初心者でも扱いやすく、ファイアウォールの設定や管理が簡単に行えます。`UFW`を使用することで、基本的なファイアウォールの設定を簡単に行い、ネットワークのセキュリティを向上させることができます。
- `iptables` とはLinuxで強力なファイアーウォールを提供するより低レイヤーのツールで`iptables`を簡単に扱えるようにしたものには他にも`firewalld` 、`shorewall`などぼちぼちある
### 特徴
- ファイアーウォールは、外からの通信に対してはデフォルトで通信拒否している。
...
### 欠点
- GUI環境に向いていない
- SSHはTCPベースのため、MiwaTsubasa1121ネットワークが不安定だと切断されやすい
### 結論
...
## 13. sudoの価値とは何ですか?
`sudo`とは一時的に管理者権限（root権限）を使用できるコマンドです
### 利点
- root権限を直接使用せずに管理者権限で操作できる（日常的にroot権限でアクセスすることは誤操作やマルウェアによるシステム破壊のリスクが上がる）
- 権限の制御ができる
- ユーザーごとのsudo利用ログの記録ができる
- ユーザーごとに権限付与できる
### 欠点
- しっかりと権限を管理するとこが重要
- ログの管理が大変
...
- **`sudo cat /var/log/sudo/sudo_config`** sudo のログを確認する
...
- **`sudo systemctl stop cron`**: `cron`停止
...
requiretty                          # TTY（仮想端末や物理端末）が必須になります。（スクリプトや自動化ツールがsudoを使用するのを防ぐ、コマンドの先にユーザーがいることを確定する）
...
- TTYを持つ
```
c6r1s4% ssh mtsubasa@localhost -p 4242 //まず、シェルに入る（TTYを得る）
mtsubasa@mtsubasa42:~$ tty　//その後コマンドを打つ
/dev/pts/0　//TTYがある
```
- TTYを持たない
```
c6r1s4% ssh mtsubasa@localhost -p 4242 "tty" //シェルスクリプトとしてコマンドを打つ（TTYを得ていない）
not a tty //TTYがない
```
